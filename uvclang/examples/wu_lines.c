// Wu's anti-aliased line algorithm. Lines fan out from the mouse position;
// move the mouse to sweep them around.
...
// Forward declarations: draw_wu_line dispatches to these before they are
// defined, and standard C requires a declaration to be in scope at the call.
void draw_45_degree_line(uint32_t *fb, uint32_t fb_width, uint32_t x0, uint32_t y0, uint32_t delta, uint32_t color);
void draw_135_degree_line(uint32_t *fb, uint32_t fb_width, uint32_t x0, uint32_t y0, uint32_t delta, uint32_t color);
void draw_wu_line_first_octant(uint32_t *fb, uint32_t fb_width, uint32_t fb_height, uint32_t x0, uint32_t y0, uint32_t x1, uint32_t y1, uint32_t color);
void draw_wu_line_second_octant(uint32_t *fb, uint32_t fb_width, uint32_t fb_height, uint32_t x0, uint32_t y0, uint32_t x1, uint32_t y1, uint32_t color);
void draw_wu_line_third_octant(uint32_t *fb, uint32_t fb_width, uint32_t fb_height, uint32_t x0, uint32_t y0, uint32_t x1, uint32_t y1, uint32_t color);
void draw_wu_line_fourth_octant(uint32_t *fb, uint32_t fb_width, uint32_t fb_height, uint32_t x0, uint32_t y0, uint32_t x1, uint32_t y1, uint32_t color);

void unsafe_alpha_blend_plot_pixel(uint32_t *fb, size_t fb_width, uint64_t x, uint64_t y, uint32_t color, uint8_t alpha)
{
    if (!alpha) return;

    uint32_t *pix_ptr = fb + fb_width * y + x;
    ...
    uint32_t dest_color = *pix_ptr;

    uint8_t unalpha = 0xFF - alpha;

    uint8_t red   = (  RED_OF(dest_color) * unalpha +   RED_OF(color) * alpha) / 0xff;
    uint8_t green = (GREEN_OF(dest_color) * unalpha + GREEN_OF(color) * alpha) / 0xff;
    uint8_t blue  = ( BLUE_OF(dest_color) * unalpha +  BLUE_OF(color) * alpha) / 0xff;

    *pix_ptr = rgb32(red, green, blue);
}

void draw_wu_line(uint32_t *fb, uint32_t fb_width, uint32_t fb_height, uint32_t x0, uint32_t y0, uint32_t x1, uint32_t y1, uint32_t color)
{
    if (x1 == x0)
    {
        if (y1 == y0)
        ...
        else
        {
            ...
            uint32_t *point = fb + x0 + y0 * fb_width;
            for (; y1 >= y0; ++y0)
            {
                *point = color;
    ...
    {
        uint32_t dx = (x1 > x0) ? (x1 - x0) : (x0 - x1);
        uint32_t dy = (y1 > y0) ? (y1 - y0) : (y0 - y1);
        if (dx == dy)
        {
            //draw_45_degree_line
            if (x1 > x0)
...
void draw_wu_line_first_octant(uint32_t *fb, uint32_t fb_width, uint32_t fb_height, uint32_t x0, uint32_t y0, uint32_t x1, uint32_t y1, uint32_t color)
{
    ...
    while (x1 >= x0)
    {
        uint8_t intensity = error_accumulator >> 8 & 0xff;
...
void draw(void)
{
    ...
    uint32_t w = FRAME_WIDTH - 1;
    uint32_t h = FRAME_HEIGHT - 1;
