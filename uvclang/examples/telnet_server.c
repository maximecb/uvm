...
uint64_t handle_conn(uint64_t sock)
{
    ...
    for (;;)
    {
        ...
        int64_t num_bytes = net_read(sock, (uint8_t *)read_buf, sizeof(read_buf) - 1);
        ...
        if (strncmp(read_buf, "exit", 4) == 0)
        {
            ...
            net_write(sock, (const uint8_t *)bye, strlen(bye));
            break;
        }

        // Echo a reply back to the client
        const char *reply = "Hello!\n";
        net_write(sock, (const uint8_t *)reply, strlen(reply));
    }
...
int main(void)
{
    ...
    int64_t listen_sock = net_listen("127.0.0.1:9000");
    ...
    for (;;)
    {
        ...
        int64_t conn_sock = net_accept(listen_sock, client_addr, sizeof(client_addr));
