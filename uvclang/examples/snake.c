// The classic Snake game. Arrow keys steer; eat apples to grow.
...
void draw_square(int xmin, int ymin, int size, uint32_t color)
{
...
void update(void)
{
    ...
    {
        printf("snake crashed into wall\n");
        exit(0);
    }
    ...
    if (nx == apple_x && ny == apple_y)
    {
        spawn_apple();
        ++snake_len;
        printf("got the apple, snake length is now %d!\n", snake_len);
    }
