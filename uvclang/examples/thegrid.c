// A retro "grid" horizon: a perspective wireframe ground plane under an animated
// sky gradient.
...
void update(void)
{
    ...
    // Draw sky/horizon. memset32 fills a whole row with one 32-bit color;
    // there is no standard-C word-fill, so this stays a UVM call.
    for (int i = 0; i < 335; ++i)
    {
        uint32_t color = hsl_to_rgb(h, s, (i / 335.0f) * l_max);
        memset32((uint32_t *)frame_buffer[i], color, FRAME_WIDTH);
    }
