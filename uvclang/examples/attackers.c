...
void draw_rect(int xmin, int ymin, int width, int height, uint32_t color);
...
uint64_t enemies_live = 0x7FFFFFFFFFFFFF;
...
void draw_dots(int xmin, int ymin, int dot_size, char* dots, uint32_t color)
{
    assert(dots);
...
void draw_rect(int xmin, int ymin, int width, int height, uint32_t color)
{
    for (int j = 0; j < height; ++j)
    {
        ...
        {
            uint32_t* pix_ptr = frame_buffer + (FRAME_WIDTH) * (ymin + j) + (xmin + i);
            *pix_ptr = color;
        }
    }
}

void draw_circle(int xmin, int ymin, int size, uint32_t color)
{
    int xmax = xmin + size;
    int ymax = ymin + size;
    int radius = size / 2;
    ...
    for (int y = ymin; y < ymax; ++y)
    {
        for (int x = xmin; x < xmax; ++x)
        {
            ...
            uint32_t* pix_ptr = frame_buffer + (FRAME_WIDTH * y + x);
            *pix_ptr = color;
        }
    }
}
...
void update_anim()
{
    ...
    for (int j = 0; j < ENEMY_ROWS; ++j)
    {
        ...
        {
            uint64_t enemy_bit = (uint64_t)1 << (ENEMY_COLS * j + i);
...
int main(void)
{
    ...
    for (uint64_t frame_idx = 0;; frame_idx = frame_idx + 1)
    {
