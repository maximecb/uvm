// A ball bouncing around the window, with a little sound effect on each bounce.
...
int16_t *audio_cb(uint64_t num_channels, uint64_t num_samples)
{
    ...
    for (uint32_t i = 0; i < num_samples && audio_pos < AUDIO_LEN; ++i)
    {
        // The intensity decreases over time
        uint32_t intensity = INT16_MAX - (audio_pos * INT16_MAX / AUDIO_LEN);

        uint32_t period = 128 - (audio_pos * 64 / AUDIO_LEN);

        uint32_t sawtooth = 4000 * (i % period) / period;
