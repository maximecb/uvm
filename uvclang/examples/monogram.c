...
uint8_t font_monogram_data[FONT_MONOGRAM_NUMBER_OF_CHARACTERS][FONT_MONOGRAM_HEIGHT] = {
...
uint32_t monogram_ascii_to_idx[127] = {
    0xFFFF,
    0xFFFF,
    0xFFFF,
    0xFFFF,
...
void draw_monogram_char(uint32_t* dest, uint32_t dest_w, char ch, uint32_t dest_x, uint32_t dest_y, uint8_t scale, uint32_t color)
{
    uint32_t char_idx = monogram_ascii_to_idx[ch];
    ...
    uint32_t* d = dest + dest_x + dest_w * dest_y;

    for (uint32_t y = 0; y < FONT_MONOGRAM_HEIGHT; ++y)
    {
        uint8_t pixel_bits = font_monogram_data[char_idx][y];
        for (uint8_t i = 0; i < scale; ++i)
        {
            uint32_t x = 0;
            uint8_t pb = pixel_bits;
...
uint32_t FRAME_WIDTH = 400;
uint32_t FRAME_HEIGHT = 400;

uint32_t frame_buffer[161600];
...
void draw()
{
    ...
    for (uint32_t ch = 32; ch < 127; ++ch) {
        uint32_t x = ch % 26 * FONT_MONOGRAM_WIDTH * scale;
        uint32_t y = ch / 26 * FONT_MONOGRAM_HEIGHT * scale;
