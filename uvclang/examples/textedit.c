...
void draw_monogram_char(uint32_t* dest, size_t dest_w, char ch, uint64_t dest_x, uint64_t dest_y, uint8_t scale, uint32_t color);
...
void redraw()
{
    memset32((uint32_t *)frame_buffer, 0x0247fe, sizeof(frame_buffer) / sizeof(uint32_t));
...
void draw_monogram_char(uint32_t* dest, size_t dest_w, char ch, uint64_t dest_x, uint64_t dest_y, uint8_t scale, uint32_t color)
{
    ...
    for (uint64_t y = 0; y < FONT_MONOGRAM_HEIGHT; ++y)
    {
