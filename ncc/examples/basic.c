...
void console_redraw_commit()
{
  ...
  for (u32 row = 0; row < NUM_ROWS; ++row)
    /* for (u32 row = line_idx? (u32)(line_idx-1): (u32)0; row <= line_idx; ++row) */
    {
...
u64* vm_command_create(u64 num) {
  ...
  for(u64** cur_cmd = vm_commands_root;; cur_cmd = (u64**)cur_cmd[VM_COMMANDS_NEXT]) {
    ...
    if(cur_cmd_num > num) { // insert command before
      ...
      break;
    } else if(cur_cmd_num == num) { // insert command exactly at
...
/* u64* double_mem(u64* m, size_t current_capacity) { */
/*   size_t new_capacity = current_capacity * 2; */
/*   u64* buff = (u64*)malloc(sizeof(u64)*new_capacity); */
/*   if(buff==NULL) return 0; */
/*   memset((u8*)buff, new_capacity, 0); */
/*   memcpy((u8*)buff, (u8*)m, current_capacity*sizeof(u64)); */
/*   free((void*)m); */
/*   return buff; */
/* } */
