...
#define memcpy(__dst, __src, __num_bytes) asm (__dst, __src, __num_bytes) -> void { syscall 3; }

// void memset(u8* dst, u8 value, u64 num_bytes)
// Fill a block of bytes in the heap with a given value.
#define memset(__dst, __value, __num_bytes) asm (__dst, __value, __num_bytes) -> void { syscall 4; }

// void print_i64(i64 val)
// Print an i64 value to standard output.
#define print_i64(__val) asm (__val) -> void { syscall 5; }

// void print_str(const char* str)
// Print a string to standard output.
#define print_str(__str) asm (__str) -> void { syscall 6; }

// void print_endl()
// Print a newline to standard output.
#define print_endl() asm () -> void { syscall 7; }

// i64 read_i64()
// Read an i64 value from standard input.
#define read_i64() asm () -> i64 { syscall 8; }

// u64 time_current_ms()
// Get the UNIX time stamp in milliseconds.
#define time_current_ms() asm () -> u64 { syscall 0; }

// void time_delay_cb(u64 delay_ms, void* callback)
// Schedule a callback to be called once after a given delay.
#define time_delay_cb(__delay_ms, __callback) asm (__delay_ms, __callback) -> void { syscall 2; }

// u32 window_create(u32 width, u32 height, const char* title, u64 flags)
// Create a new window with a frame buffer to draw into.
#define window_create(__width, __height, __title, __flags) asm (__width, __height, __title, __flags) -> u32 { syscall 1; }

// void window_show(u32 window_id)
// Show a window, initially not visible when created.
#define window_show(__window_id) asm (__window_id) -> void { syscall 9; }
...
#define window_draw_frame(__window_id, __pixel_data) asm (__window_id, __pixel_data) -> void { syscall 10; }

// void window_on_mousemove(u32 window_id, void* callback)
// Register a callback for mouse movement.
#define window_on_mousemove(__window_id, __callback) asm (__window_id, __callback) -> void { syscall 11; }

// void window_on_mousedown(u32 window_id, void* callback)
// Register a callback for mouse button press events.
#define window_on_mousedown(__window_id, __callback) asm (__window_id, __callback) -> void { syscall 12; }

// void window_on_mouseup(u32 window_id, void* callback)
// Register a callback for mouse button release events.
#define window_on_mouseup(__window_id, __callback) asm (__window_id, __callback) -> void { syscall 13; }
